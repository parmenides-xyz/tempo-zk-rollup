...
impl RollupContract {
    ...
    pub async fn listen_for_validator_set_added(
        ...
        // Poll eth_getLogs instead of using eth_newFilter (not supported by Tempo)
        let stream = async_stream::stream! {
            // Start from current block to avoid exceeding max block range
            let mut last_block: u64 = match client.eth().block_number().await {
                Ok(n) => n.as_u64(),
                Err(_) => 0,
            };
            loop {
                ...
                let current_block = match client.eth().block_number().await {
                    Ok(n) => n.as_u64(),
                    Err(e) => {
                        yield Err(e);
                        continue;
                ...
                let filter = FilterBuilder::default()
                    ...
                    .from_block(web3::types::BlockNumber::Number(last_block.into()))
                    .to_block(web3::types::BlockNumber::Number(current_block.into()))
                    .build();

                match client.eth().logs(filter).await {
                    Ok(logs) => {
                        for log in logs {
                            yield Ok(log);
                        }
